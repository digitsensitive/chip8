...
void Chip8::init() {
  ...
  currentOpcode = 0;  // Reset current opcode
...
void Chip8::emulateCycle() {
  ...
  switch (currentOpcode & 0xF000) {
      ...
      switch (currentOpcode & 0x000F) {
        ...
        case 0x000E:  // 0x00EE: Returns from subroutine
          --sp;       // 16 levels of stack, decrease stack pointer to prevent
                      // overwrite
          pc = stack[sp];  // Put the stored return address from the stack back
    ...
    case 0x2000:       // 0x2NNN: Calls subroutine at NNN.
      ...
      pc = currentOpcode &
           0x0FFF;  // Set the program counter to the address at NNN
      break;
    ...
    case 0x8000:
      switch (currentOpcode & 0x000F) {
        ...
        case 0x0004:  // 0x8XY4: Adds VY to VX. VF is set to 1 when there's a
          ...
          if (V[(currentOpcode & 0x00F0) >> 4] >
              (0xFF - V[(currentOpcode & 0x0F00) >> 8]))
            V[0xF] = 1;  // carry
          else
            V[0xF] = 0;
          V[(currentOpcode & 0x0F00) >> 8] += V[(currentOpcode & 0x00F0) >> 4];
        ...
        case 0x0006:  // 0x8XY6: Shifts VX right by one. VF is set to the value
        ...
        case 0x0007:  // 0x8XY7: Sets VX to VY minus VX. VF is set to 0 when
                      // there's a borrow, and 1 when there isn't
          if (V[(currentOpcode & 0x0F00) >> 8] >
              V[(currentOpcode & 0x00F0) >> 4])  // VY-VX
            V[0xF] = 0;                          // there is a borrow
          else
            V[0xF] = 1;
          V[(currentOpcode & 0x0F00) >> 8] = V[(currentOpcode & 0x00F0) >> 4] -
        ...
        case 0x000E:  // 0x8XYE: Shifts VX left by one. VF is set to the value
    ...
    case 0xC000:  // CXNN: Sets VX to a random number and NN
      V[(currentOpcode & 0x0F00) >> 8] =
          (rand() % 0xFF) & (currentOpcode & 0x00FF);
      pc += 2;
      break;
    ...
    case 0xD000:  // DXYN: Draws a sprite at coordinate (VX, VY) that has a
                  ...
                  // instruction. VF is set to 1 if any screen pixels are
    ...
    case 0xE000:
      switch (currentOpcode & 0x00FF) {
        case 0x009E:  // EX9E: Skips the next instruction if the key stored in
                      // VX is pressed
        ...
        case 0x00A1:  // EXA1: Skips the next instruction if the key stored in
                      // VX isn't pressed
          if (key[V[(currentOpcode & 0x0F00) >> 8]] == 0)
            pc += 4;
          else
    ...
    case 0xF000:
      switch (currentOpcode & 0x00FF) {
        ...
        case 0x001E:  // FX1E: Adds VX to I
          if (I + V[(currentOpcode & 0x0F00) >> 8] >
              0xFFF)  // VF is set to 1 when range overflow (I+VX>0xFFF), and 0
                      // when there isn't.
            V[0xF] = 1;
          else
            V[0xF] = 0;
        ...
        case 0x0033:  // FX33: Stores the Binary-coded decimal representation of
          ...
          memory[I] = V[(currentOpcode & 0x0F00) >> 8] / 100;
          memory[I + 1] = (V[(currentOpcode & 0x0F00) >> 8] / 10) % 10;
          memory[I + 2] = (V[(currentOpcode & 0x0F00) >> 8] % 100) % 10;
          pc += 2;
          break;
        ...
        case 0x0055:  // FX55: Stores V0 to VX in memory starting at address I
          ...
          // + 1.
          I += ((currentOpcode & 0x0F00) >> 8) + 1;
          pc += 2;
          break;

        case 0x0065:  // FX65: Fills V0 to VX with values from memory starting
                      // at address I
          for (int i = 0; i <= ((currentOpcode & 0x0F00) >> 8); ++i)
            V[i] = memory[I + i];
